...
const btnPrimary =
  "inline-flex h-10 w-full items-center justify-center rounded-md bg-teal-700 px-4 text-sm font-medium text-white transition hover:bg-teal-800 disabled:opacity-50 disabled:cursor-not-allowed";
const btnSecondary =
  "inline-flex h-10 w-full items-center justify-center rounded-md border border-zinc-200 bg-white px-4 text-sm font-medium text-zinc-600 transition hover:bg-zinc-50 disabled:opacity-50 disabled:cursor-not-allowed dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-300 dark:hover:bg-zinc-900";
const btnDark =
  "inline-flex h-10 w-full items-center justify-center rounded-md bg-zinc-800 px-4 text-sm font-medium text-white transition hover:bg-zinc-700 disabled:opacity-50 disabled:cursor-not-allowed dark:bg-zinc-200 dark:text-zinc-900 dark:hover:bg-zinc-300";
...
function RequestPanel({ request, now }: { request: ActiveRequest; now: Date }) {
  ...
  if (status === "AWAITING_PAYMENT") {
    return (
      <div className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        <div className="mb-1 text-xs font-medium uppercase tracking-wide text-zinc-400">Pending payment</div>
        ...
        <div className="mt-5 flex flex-col gap-2 sm:flex-row">
          ...
          <form action={cancelRequest} className="flex-1">
            ...
            <SubmitButton className={btnSecondary} pendingText="Cancelling…">Cancel</SubmitButton>
          </form>
        </div>
      </div>
    );
  ...
  if (status === "PAID" && key) {
    return (
      <div className="rounded-lg border border-teal-200 bg-teal-50 p-6 shadow-sm dark:border-teal-900/70 dark:bg-teal-950/20">
        <div className="mb-1 text-xs font-medium uppercase tracking-wide text-teal-700 dark:text-teal-400">Key ready for pickup</div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          ...
          <div className="rounded-md bg-white px-4 py-3 shadow-sm dark:bg-zinc-950">
            ...
            <div className="mt-0.5 text-xl font-semibold tabular-nums tracking-widest">{key.cabinet.currentCode}</div>
          </div>
        </div>
        ...
        <form action={markPickedUp} className="mt-4">
          <input name="id" type="hidden" value={id} />
          <SubmitButton className={btnPrimary} pendingText="Confirming…">I&apos;ve picked it up</SubmitButton>
        </form>
      </div>
    );
  }
  ...
  if (status === "PICKED_UP" && request.dueAt && key) {
    ...
    return (
      <div className={`rounded-lg border p-6 shadow-sm ${isOverdue ? "border-red-200 bg-red-50 dark:border-red-900/70 dark:bg-red-950/20" : "border-amber-200 bg-amber-50 dark:border-amber-900/70 dark:bg-amber-950/20"}`}>
        <div className={`mb-1 text-xs font-medium uppercase tracking-wide ${isOverdue ? "text-red-700 dark:text-red-400" : "text-amber-700 dark:text-amber-400"}`}>
        ...
        {isOverdue ? (
          ...
          <div className="mt-2 text-sm text-amber-700 dark:text-amber-400">Return by {dtFmt.format(request.dueAt)}</div>
        )}
        ...
        <form action={returnKey} className="mt-4">
          <input name="id" type="hidden" value={id} />
          <SubmitButton className={btnDark} pendingText="Returning…">Return key</SubmitButton>
        </form>
      </div>
    );
  }
...
function HistorySection({ requests }: { requests: PastRequest[] }) {
  ...
  return (
    <section className="mt-8">
      <h2 className="mb-3 text-xs font-medium uppercase tracking-wide text-zinc-400">Past requests</h2>
  ...
  return (
    <main className="mx-auto w-full max-w-xl flex-1 px-6 py-12">
      <header className="mb-8 flex items-start justify-between">
        ...
        <form action={logout}>
          <SubmitButton className="text-sm text-zinc-400 hover:text-zinc-700 disabled:opacity-50 dark:hover:text-zinc-200" pendingText="…">
            Sign out
          </SubmitButton>
        </form>
      </header>
      ...
      {activeRequest ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          >
            ...
            <p className="mt-2 text-sm text-zinc-500">Get temporary access to your apartment. €20.00 · 6-hour hold.</p>
            <div className="mt-4 text-xs font-medium text-teal-700 group-hover:underline dark:text-teal-400">Start request →</div>
          </Link>
          <Link
            className="group rounded-lg border border-zinc-200 bg-white p-5 shadow-sm transition hover:border-teal-300 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-950"
          ...
          >
            ...
            <p className="mt-2 text-sm text-zinc-500">Request a key on behalf of another tenant in the building.</p>
            <div className="mt-4 text-xs font-medium text-teal-700 group-hover:underline dark:text-teal-400">Start request →</div>
          </Link>
        </div>
      )}
